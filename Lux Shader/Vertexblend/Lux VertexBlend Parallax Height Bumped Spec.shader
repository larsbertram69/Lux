Shader "Lux/VertexBlend/Parallax Height Bumped Specular" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_SpecTex ("Specular Color (RGB) Roughness (A)", 2D) = "black" {}
		_BumpMap ("Normalmap", 2D) = "bump" {}

		_MainTex1 ("Base Tex1(RGB) Height(A)", 2D) = "white" {}
	...
	//	#define LUX_AO_OFF
		...
		float _Parallax, _Tex1TilingX, _Tex1TilingY;
		...
		void surf (Input IN, inout SurfaceOutputLux o) {
			...
			// Parallax
			half h = tex2D(_MainTex1, uv_tex1).a;
			...
			float2 blendval = float2(1.0-IN.color.r, IN.color.r);
